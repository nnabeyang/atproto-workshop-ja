...
両者の違いとして、ファイアホースはデータが **バイナリ形式（CBOR）** で送出されるため、今回の演習のようなコマンドライン操作でそのまま扱うには不向きです。一方、ジェットストリームはすべて **テキストベースの JSON 形式**で提供されているため、`jq` などのコマンドで手軽にパースすることができます。詳細については、https://atproto.com/guides/streaming-data のガイドを参照してください。
...
さあ、いよいよ面白くなってきました。ここでは、完全な at:// URI 構造と、AT レコードに含まれる内容を確認できます。今回の例では、ブラウザで開くために意図的に `app.bsky.feed.post` レコードを抽出しましたが、このプロトコルには他にも多くの **レキシコン（Lexicon）** が存在します。ネットワーク上のレコードの大部分は、実際には Bluesky の「いいね（likes）」ですが、 Bluesky 以外で定義されたレキシコンのデータもたくさん流れています！
